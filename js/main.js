...
if (audioEl) audioEl.volume = 0.22;
...
function dibujarConst() {
  ...
  ctx.strokeStyle = 'rgba(255,105,180,.22)';
  ctx.lineWidth = 1;
  lineas.forEach(([a, b]) => {
    const ea = estrellas[a], eb = estrellas[b];
    ctx.beginPath();
    ctx.moveTo(ea.x * W, ea.y * H);
    ctx.lineTo(eb.x * W, eb.y * H);
    ctx.stroke();
  });
  ...
  estrellas.forEach(e => {
    ...
    ctx.font = `bold ${r * .9}px Cinzel, serif`;
...
document.addEventListener('DOMContentLoaded', () => {
  const canvas = document.getElementById('cv-const');
  if (!canvas) return;
...
function buildProg() {
  const p = document.getElementById('prog-dots');
  if (!p) return;
  p.innerHTML = '';
  pistas.forEach((_, i) => {
    const d = document.createElement('div');
    d.className = 'pdot' + (i === 0 ? ' act' : '');
    d.id = 'pd' + i;
    p.appendChild(d);
  });
}
...
function mostrarPista() {
  ...
  const ops = document.getElementById('esc-opciones');
  ops.innerHTML = '';
  p.ops.forEach((op, i) => {
    const b = document.createElement('button');
    b.className = 'esc-btn';
    b.textContent = op;
    b.onclick = () => responder(i, b, ops);
    ops.appendChild(b);
  });
}
...
function abrirCajita() {
  ...
  if (tapa) tapa.classList.add('abierta');
  if (hint) hint.style.display = 'none';
  setTimeout(() => {
    if (anilloInside) anilloInside.classList.remove('hidden');
  }, 400);
  setTimeout(() => {
    const preg = document.getElementById('anillo-pregunta');
    if (preg) preg.classList.remove('hidden');
  }, 900);
}

function aceptarAnillo() {
  const panel = document.getElementById('panel-anillo');
...
function irFinalAmor() {
  ...
  cajitaAbierta = false;
  const tapa = document.getElementById('cajita-tapa');
  const inside = document.getElementById('anillo-inside');
  const preg = document.getElementById('anillo-pregunta');
  const hint = document.getElementById('cajita-hint');
  if (tapa) tapa.classList.remove('abierta');
...
function iniciarCuentaRegresiva() {
  ...
  function tick() {
    ...
    const elD = document.getElementById('cr-dias');
    const elH = document.getElementById('cr-horas');
    const elM = document.getElementById('cr-min');
    const elS = document.getElementById('cr-seg');
    if (!elD) return;
    if (diff <= 0) {
      elD.textContent = '0';
      elH.textContent = '0';
    ...
    elH.textContent = Math.floor((diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    elM.textContent = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
    elS.textContent = Math.floor((diff % (1000 * 60)) / 1000);
  }
  tick();
...
function mostrarExtra(id) {
  ...
  if (panel) {
    ...
    if (id === 'panel-ruleta') {
      const msg = document.getElementById('ruleta-msg');
      if (msg) {
        msg.textContent = 'Presiona girar para recibir tu mensaje... ✨';
        msg.style.opacity = '1';
      }
    }
